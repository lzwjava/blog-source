---
title: Building a Python MCP Weather Server
lang: en
layout: post
audio: false
translated: false
...
1. Install `uv` (a fast Python project manager):
   ```
   curl -LsSf https://astral.sh/uv/install.sh | sh
   ```
   Restart your terminal to make `uv` available.
...
2. Configure the app by creating/editing `~/Library/Application Support/Claude/claude_desktop_config.json` (macOS) or the equivalent on Windows (`%APPDATA%\Claude\claude_desktop_config.json`):
   ```json
   {
     "mcpServers": {
       "weather": {
         ...
         "args": [
           "--directory",
           "/ABSOLUTE/PATH/TO/weather",  # Replace with your project path (use pwd to find it)
           "run",
           "weather.py"
         ]
       }
   ...
   ```
   - Use absolute paths (e.g., `/Users/yourname/weather` on macOS).
...
This sets up a functional MCP server. For TypeScript/Node.js alternatives or more examples, check the resources below.
