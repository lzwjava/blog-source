...
def code_to_pdf(input_dir, output_file):
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith((".py", ".js", ".java", ".c", ".cpp", ".go", ".sh", ".html", ".css")):
    ...
    try:
        subprocess.run(
            ['pandoc', temp_text_file, '-o', output_file, '--from', 'txt', '--pdf-engine=xelatex'],
            check=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", required=True, help="The directory containing the code files.")
