            ...
            for file_path in file_paths:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    if "](assets" in content:
                        ...
                        content, count = re.subn(r'{:\s*\.centered\s*}\s*(!\[.*?\])(\s*\*.*?\*{:\s*\.caption\s*})?', replace_centered_image, content)
                        if count > 0:
                            print(f"Replaced {count} centered image tags with optional captions.")

                        # Handle responsive images
                        content = re.sub(r'(!\[.*?\]){:\s*\.responsive\s*}', lambda match: replace_image_tag(match) if match else "", content)
                        count = len(re.findall(r'!\[.*?\]', content))
                        if count > 0:
                            print(f"Replaced responsive image tags.")

                        # Remove captions
                        content, count = re.subn(r'\*.*?\*{:\s*\.caption\s*}', '', content)
                        if count > 0:
...
def main():
    ...
    parser.add_argument("input_path",  help="Path to the directory containing markdown files.")
