...
def _get_ec2_instances():
    ...
    try:
        result = subprocess.run(["aws", "ec2", "describe-instances", "--region", "ap-east-1"], capture_output=True, text=True, check=True)
...
def _get_ec2_instance(instance_id):
    ...
    try:
        result = subprocess.run(["aws", "ec2", "describe-instances", "--region", "ap-east-1", "--instance-ids", instance_id], capture_output=True, text=True, check=True)
        instance_data = yaml.safe_load(result.stdout)
        if not instance_data or 'Reservations' not in instance_data or not instance_data['Reservations'] or 'Instances' not in instance_data['Reservations'][0] or not instance_data['Reservations'][0]['Instances']:
...
def create_ec2_instance(instance_name=None, availability_zone="ap-east-1a", instance_type="t3.micro", user_data=None):
    ...
    if not user_data:
        ...
        sudo apt update
        """
    print(f"Creating EC2 instance with name: {instance_name}, zone: {availability_zone}, type: {instance_type}...")
...
def delete_all_ec2_instances(instance_id=None):
    if instance_id:
        ...
        print(f"Executing command: aws ec2 terminate-instances --instance-ids {instance_id} --region ap-east-1")
        try:
            subprocess.run(["aws", "ec2", "terminate-instances", "--instance-ids", instance_id, "--region", "ap-east-1"], check=True)
