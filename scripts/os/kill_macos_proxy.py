                ...
                if any(pattern in line_lower for pattern in clash_patterns):
                    ...
                    if len(parts) >= 11:
                        ...
                        if 'grep' not in command and user != 'root':
                            ...
                            if ('clash-darwin-amd64' in command or
                                'clash.py' in command or
                                ('/python' in command and 'clash' in command)):
...
def main():
    ...
    if not args.force:
        print(f"Do you want to kill all {len(processes)} Clash process(es)? (Press Enter to kill, or 'no' to exit)")
