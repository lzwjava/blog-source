...
def update_front_matter(original_file_path, translated_file_path):
    try:
        ...
        original_front_matter_match = re.match(r'---\n(.*?)\n---', original_content, re.DOTALL)
        original_front_matter = original_front_matter_match.group(1) if original_front_matter_match else None
        
        if not original_front_matter:
            print(f"No front matter found in {original_file_path}")
            return

        original_front_matter_dict = yaml.safe_load(original_front_matter) if original_front_matter else {}
        ...
        if 'lang' in original_front_matter_dict and 'lang' in translated_front_matter_dict:
            if original_front_matter_dict['lang'] != translated_front_matter_dict['lang']:
