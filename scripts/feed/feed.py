...
audio_files = [f for f in os.listdir(audio_dir) if f.endswith('.mp3') and f.split('-')[-1].split('.')[0] == lang]
...
for pub_date, audio_file in episodes:  # Episodes are already sorted by pubdate desc
    ...
    if os.path.exists(post_path):
        ...
        try:
            ...
            title = ' '.join(title_parts).replace('-', ' ').title()  # Fallback to filename-based title
    else:
        title = ' '.join(title_parts).replace('-', ' ').title()  # Fallback to filename-based title
