...
def get_gpu_info():
    ...
    nvidia_cmd = run_command("nvidia-smi --query-gpu=name,memory.total,memory.used,memory.free --format=csv,noheader,nounits")
...
def get_cuda_info():
    ...
    cuda_version = run_command("nvidia-smi --query-gpu=cuda_runtime_version --format=csv,noheader,nounits")
    ...
    nvcc_version = run_command("nvcc --version | grep -i 'release' | awk '{print $6}' | cut -d',' -f1")
    if nvcc_version and nvcc_version != "":
        cuda_versions.append(f"NVCC Compiler: {nvcc_version}")

    # Check cuDNN version (common location)
    cudnn_version = run_command("cat /usr/include/cudnn_version.h 2>/dev/null | grep -w CUDNN_MAJOR | awk '{print $3}' | tr -d ';' | xargs")
    if cudnn_version:
        cudnn_minor = run_command("cat /usr/include/cudnn_version.h 2>/dev/null | grep -w CUDNN_MINOR | awk '{print $3}' | tr -d ';'")
        cudnn_patch = run_command("cat /usr/include/cudnn_version.h 2>/dev/null | grep -w CUDNN_PATCHLEVEL | awk '{print $3}' | tr -d ';'")
...
def get_vulkan_info():
    ...
    vulkan_icd = run_command("vulkaninfo --summary 2>/dev/null | grep -i 'device name' | head -3")
    if vulkan_icd:
        vulkan_info.append(f"Vulkan ICD: {vulkan_icd}")

    # Check Vulkan SDK version
    vulkan_sdk = run_command("vulkaninfo --summary 2>/dev/null | grep -i 'vulkan' | head -1")
    if vulkan_sdk:
