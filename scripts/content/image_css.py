...
def is_image_responsive(image_path):
    ...
    try:
        command = ["ffprobe", "-v", "error", "-select_streams", "v:0",
                   "-show_entries", "stream=width,height", "-of", "csv=s=,:p=0", image_path]
...
def process_markdown_files(directory):
    ...
    for filename in os.listdir(directory):
        if filename.endswith(".md"):
            ...
            image_pattern = re.compile(r'!\[.*?\]\((.*?)\)(?:\{: \.responsive \})?(?:\{: \.caption \})?')
                ...
                if os.path.exists(full_image_path):
                    ...
                    if is_responsive:
                        if "{: .responsive }" not in match.group(0):
                            ...
                            updated_content = updated_content.replace(match.group(0), f"{match.group(0)}{{: .responsive }}")
                    else:
                        if "{: .responsive }" in match.group(0):
                            print(f"Remove responsive class from {image_path} in {filename}")
                            updated_content = updated_content.replace(f"{match.group(0)}{{: .responsive }}", match.group(0))
                            updated_content = updated_content.replace(f"{match.group(0)}{{: .responsive }}", match.group(0))
