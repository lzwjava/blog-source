...
def update_front_matter(original_file_path, translated_file_path, is_translated):
    try:
...
def main():
    ...
    for filename in os.listdir(original_dir):
        ...
        en_file_path = os.path.join(posts_dir, 'en', f"{base_name}-en.md")
        zh_file_path = os.path.join(posts_dir, 'zh', f"{base_name}-zh.md")
        ...
        for lang_dir in lang_dirs:
            ...
            is_translated = False
            if os.path.exists(en_file_path) or os.path.exists(zh_file_path):
                 is_translated = True
            
            if os.path.exists(translated_file_path):
                update_front_matter(original_file_path, translated_file_path, is_translated)
        ...
        if filename.endswith("-en.md") or filename.endswith("-zh.md"):
            base_name_without_suffix = filename.replace("-en.md", "").replace("-zh.md", "")
            
            en_file_path = os.path.join(posts_dir, 'en', f"{base_name_without_suffix}-en.md")
            zh_file_path = os.path.join(posts_dir, 'zh', f"{base_name_without_suffix}-zh.md")
            
            for lang_dir in lang_dirs:
                translated_file_name = f"{base_name_without_suffix}-{lang_dir}.md"
                translated_file_path = os.path.join(posts_dir, lang_dir, translated_file_name)
