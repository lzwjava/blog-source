...
plt.plot(df['Date'], df['Left Eye Myopia'], marker='o', label='Left Eye Myopia', color='blue')

# Plot Right Eye Myopia
plt.plot(df['Date'], df['Right Eye Myopia'], marker='o', label='Right Eye Myopia', color='red')

# Plot Left Eye Astigmatism
plt.plot(df['Date'], df['Left Eye Astigmatism'], marker='s', label='Left Eye Astigmatism', color='green')

# Plot Right Eye Astigmatism
plt.plot(df['Date'], df['Right Eye Astigmatism'], marker='s', label='Right Eye Astigmatism', color='purple')
...
for idx, (date, vision) in enumerate(zip(df['Date'], correct_vision)):
    plt.annotate(f'CV: {vision}', (date, min_astigmatism - 0.5),
...
plt.title("Zhiwei Li's Progress in Reversing Myopia and Astigmatism Naturally", fontsize=16, fontweight='bold')
