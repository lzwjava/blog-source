---
audio: false
generated: false
image: false
lang: hi
layout: post
title: सदस्यता लें
translated: true
type: post
---

मेरे नवीनतम ब्लॉग पोस्ट से अपडेट रहने के लिए आरएसएस फीड, ऑडियो फीड के माध्यम से सब्सक्राइब करें या मुझे एक्स (ट्विटर) पर फॉलो करें।  

- आरएसएस फीड:  
  इस लिंक का उपयोग करके सब्सक्राइब करें: [https://lzwjava.github.io/feed/feed.xml](https://lzwjava.github.io/feed/feed.xml)।  

- ऑडियो फीड:  
  इस लिंक का उपयोग करके सब्सक्राइब करें: [https://lzwjava.github.io/feed/audio-feed.xml](https://lzwjava.github.io/feed/audio-feed.xml)।  

- कन्वर्सेशन फीड:  
  इस लिंक का उपयोग करके सब्सक्राइब करें: [https://lzwjava.github.io/feed/conversation-feed.xml](https://lzwjava.github.io/feed/conversation-feed.xml)।  

- एक्स (ट्विटर):  
  अपडेट्स और अधिक के लिए मुझे [@lzwjava](https://mobile.twitter.com/lzwjava) पर फॉलो करें।  

इसके अलावा, यहां मेरे कुछ प्लेटफॉर्म अकाउंट्स हैं:  

| प्लेटफॉर्म   | लिंक                                        |
|----------------|-------------------------------------------------|
| कोड       | [GitHub](https://github.com/lzwjava)            |
|                | [Codeforces](https://codeforces.com/profile/lzwjava) |
|                | [UVa Judge](https://uhunt.onlinejudge.org/id/113519) |
|                | [ब्लॉग](https://lzwjava.github.io)               |
|                | [Kaggle](https://www.kaggle.com/lzwjava)        |
|                | [LeetCode](https://leetcode.com/u/lzwjava/)     |
|                | [AtCoder](https://atcoder.jp/users/lzwjava)     |
|                | [SPOJ](https://www.spoj.com/users/lzwjava)      |
|                | [HackerRank](https://www.hackerrank.com/profile/lzwjava) |
|                | [CodeChef](https://www.codechef.com/users/lzwjava) |
|                | [CSDN](https://blog.csdn.net/lzw_java)          |
|                | [StackOverflow](https://stackoverflow.com/users/3044511/lzwjava) |
| सोशल     | [Weibo](https://weibo.com/zhiweilee)            |
|                | [Twitter](https://twitter.com/lzwjava)          |
|                | [LinkedIn](https://www.linkedin.com/in/lzwjava) |
|                | [Facebook](https://www.facebook.com/lzwjava)    |
|                | [Xiaohongshu](https://www.xiaohongshu.com/user/profile/577ba85b6a6a6913f98f237b) |
|                | [BlueSky](https://bsky.app/profile/lzwjava.bsky.social) |
|                | [Threads](https://www.threads.net/@lzwjava)     |
| वीडियो      | [Douban](https://www.douban.com/people/lzwjava/) |
|                | [TikTok](https://www.tiktok.com/@jamesleecool)  |
|                | [Youtube](https://www.youtube.com/channel/UC-jqwnFwWOS4ydgTsQR-9ZQ) |
| मैसेजिंग  | [Telegram](https://t.me/lzwjava)               |
|                | [WhatsApp](https://wa.me/12345678900)          |
| अन्य      | [Quora](https://www.quora.com/profile/Lzw-2)    |