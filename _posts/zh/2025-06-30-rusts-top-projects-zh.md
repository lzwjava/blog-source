...
* **Rust（语言本体）：** 官方仓库（`rust-lang/rust`）自然是核心中的核心，包含编译器、标准库等关键组件
* **Cargo：** Rust 包管理器与构建系统，是开发体验中不可或缺的一环
