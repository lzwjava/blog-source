...
字符串无处不在，高效的字符串操作是一项关键技能。让我们来看看KMP算法、字典树数据结构和Rabin-Karp用于模式匹配。

## 1. KMP算法：高效字符串匹配
Knuth-Morris-Pratt（KMP）算法预处理模式以避免不必要的比较，实现O(n + m)时间。
...
## 2. 字典树：前缀查找
字典树以树的形式存储字符串，用于快速前缀查找，空间复杂度与总字符数成比例。
...
Rabin-Karp使用哈希查找模式，平均时间为O(n + m)，但最坏情况为O(nm)。
