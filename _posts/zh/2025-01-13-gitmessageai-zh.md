...
这个 Python 脚本应该放在包含在系统 PATH 中的目录中，例如 `~/bin`。
...
def call_gemini_api(prompt):
    ...
    try:
        ...
        response.raise_for_status()  # 对于不良状态码引发异常
        response_json = response.json()
...
def gitmessageai(push=True, only_message=False, api='deepseek'):
    ...
    # 为 AI 准备提示
    prompt = f"""
生成以下代码更改的简洁提交消息，使用 Conventional Commits 格式。
...
"""
    ...
    if api == 'deepseek':
        ...
        if not commit_message:
            print("错误：Gemini API 没有响应。")
            return
    elif api == 'mistral':
        commit_message = call_mistral_api(prompt)
        if not commit_message:
            print("错误：Mistral API 没有响应。")
            return
    else:
...
* 一个是只发送文件名更改，不使用 `git diff` 读取文件的详细更改。我们不想向 AI 服务 API 提供太多细节。在这种情况下，我们不需要它，因为很少有人会仔细阅读提交消息。
