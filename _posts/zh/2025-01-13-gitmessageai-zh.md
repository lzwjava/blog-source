...
这个 Python 脚本应该放置在包含在系统 PATH 中的目录中，例如 `~/bin`。
...
* 一个改进是只发送文件名更改，而不使用 `git diff` 读取文件的详细更改。我们不想给 AI 服务 API 提供太多细节。在这种情况下，我们不需要它，因为很少有人会仔细阅读提交信息。

* 有时，Deepseek API 会失败，因为它最近非常受欢迎。我们可能需要使用 Gemini 作为替代。