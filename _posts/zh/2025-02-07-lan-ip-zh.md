...
这个Python脚本扫描局域网以查找活动的IP地址。它使用`ping`命令检查主机是否可达，并使用多线程来加快扫描速度。信号量限制并发线程的数量，以避免压垮系统。该脚本接受网络地址（例如，“192.168.1.0/24”）作为输入，并打印网络中每个IP地址的状态（在线或离线）。
...
该脚本识别活动的IP地址。为确保正确的网络通信，请验证是否已配置代理设置以跳过这些本地IP。
