...
这个Python脚本扫描本地网络以查找活动的IP地址。它使用`ping`命令检查主机是否可达，并使用多线程来加快扫描过程。信号量限制并发线程的数量，以避免压垮系统。脚本接受网络地址（例如，“192.168.1.0/24”）作为输入，并打印网络中每个IP地址的运行状态（up或down）。

此脚本通过扫描活动IP地址来帮助识别网络上的设备，例如在有线桥接模式下运行的TP-LINK网状路由器。
...
def is_host_up(host, port=None):
    """
    使用ping或telnet检查主机是否启动。
    如果指定了端口，则使用telnet检查端口是否打开。
    否则，使用ping。
    如果主机启动则返回True，否则返回False。
    """
    if port:
        try:
...
脚本识别活动IP地址。为确保正确的网络通信，请验证是否已配置代理设置以跳过这些本地IP。
...
因为32 - 27 = 5，2^5 = 32，所以它将尝试`192.168.1.0`到`192.168.1.31`。

但是，使用`192.168.1.0/28`时它不起作用，因为2^4 = 16，所以它将尝试`192.168.1.0`到`192.168.1.15`，这并不包含`192.168.1.16`。
