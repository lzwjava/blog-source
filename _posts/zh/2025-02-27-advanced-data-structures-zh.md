...
数据结构是高效算法的基础。让我们探索四种强大的数据结构：跳表、并查集、AVL树和二进制索引树。这些在需要快速搜索、合并、平衡或范围查询的场景中广泛使用。

## 1. 跳表：概率搜索
跳表是一种分层链表，允许快速搜索、插入和删除，平均时间复杂度为O(log n)，是平衡树的一种替代方案。
...
## 2. 并查集（Disjoint Set）：连通性跟踪
并查集高效管理不相交集合，支持合并和查找操作，平均时间复杂度几乎为O(1)，使用路径压缩和秩启发式。
...
## 3. AVL 树：自平衡二叉搜索树
AVL 树是一种自平衡二叉搜索树，子树高度差（平衡因子）最多为1，确保O(log n)操作。
...
```
**输出:** `前序遍历: 30 20 10 25 40 50`

## 4. 二进制索引树（Fenwick Tree）：范围查询
二进制索引树（BIT）高效处理范围求和查询和更新，时间复杂度为O(log n)，常用于竞赛编程。
...
public class BinaryIndexedTree {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("2 到 5 的范围和: " + bit.rangeSum(2, 5));
        bit.update(3, 6); // 将 6 添加到索引 3
        System.out.println("新的 2 到 5 的范围和: " + bit.rangeSum(2, 5));
    }
}
```
**输出:**
...
## 2. 蒙特卡罗模拟：概率估计
蒙特卡罗方法使用随机采样来估算结果，例如通过模拟正方形和圆内的点来近似 π。
...
public class MonteCarlo {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("使用 " + points + " 个点估算的 π: " + pi);
