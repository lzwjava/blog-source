...
### فهم الطلب
- **عندما يكون `level = 0` أو غير موفر**: استخدم الاسم الكامل للحزمة كما تم استخراجها من بياتات `import` في Java (مثل `java.util` أو `com.google.common.eventbus`) دون تقصيرها.
...
- يجب أن يستمر النص في معالجة ملفات Java، استخراج أسماء الحزم من بياتات `import`، ومعاملتها بشكل مناسب.

### طريقة الحل
1. **تحليل المعلمات**:
   - إذا تم توفير فقط الدليل الجذر (مثلًا `python script.py /path/to/dir`)، قم بتعيين `level = 0`، مما يعني استخدام الاسم الكامل للحزمة.
   - إذا تم توفير `level` (مثلًا `python script.py /path/to/dir 2`)، استخدمه لتقصير اسم الحزمة، مع التأكد من أنه عدد صحيح موجب.
...
2. **استخراج الحزمة**:
   ...
   - استخدام معايير تسمية Java: الحزم عادة ما تكون بأحرف صغيرة، وأسماء الفئات تبدأ بحرف كبير.

3. **منطق تقصير الحزمة**:
   - إذا كان `level = 0`، أضف الاسم الكامل للحزمة إلى مجموعة النتائج.
   - إذا كان `level > 0`، قم بفصل اسم الحزمة بنقاط (`.`) وخذ أول `level` أجزاء.
...
def extract_package(import_statement):
    ...
    يُفترض أن أسماء الحزم بأحرف صغيرة وتوقف عند أسماء الفئات (أحرف كبيرة) أو النجمات (*).
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        root_dir = sys.argv[1]
        level = 0  # لم يتم توفير مستوى، استخدم أسماء الحزم الكاملة
    elif len(sys.argv) == 3:
        root_dir = sys.argv[1]
        try:
    ...
    if packages:
        if level == 0:
            print("[INFO] تم الانتهاء من التحليل. طباعة الحزم الفريدة الكاملة:")
        else:
            print(f"[INFO] تم الانتهاء من التحليل. طباعة الحزم الفريدة على مستوى {level}:")
...
- **مثال على الإخراج**:
  ...
  - **مع `level = 0` (أو بدون مستوى موفر)**:
    ...
    [INFO] تم الانتهاء من التحليل. طباعة الحزم الفريدة الكاملة:
  ...
  - **مع `level = 2`**:
    ...
    [INFO] تم الانتهاء من التحليل. طباعة الحزم الفريدة على مستوى 2:
    com.google
    java.util
    ```
...
### حالات حافة معالجتها
- **العدم وجود المستوى**: الافتراض `level = 0`، واستخدام أسماء الحزم الكاملة.
- **العدم وجود المستوى**: الخروج بالخطأ إذا كان `level` سالبًا أو غير عدد صحيح.
- **الحزم القصيرة**: إذا كانت الحزمة أقل من `level` (مثل `java` مع `level = 2`)، يستخدم الاسم الكامل (`java`).
- **الاستيرادات الفارغة**: تخطي السطور التي لم يتم استخراج الحزمة منها.

يفي هذا النص بالكامل بطلبك: عندما يكون `level = 0` أو غير موفر، يستخدم جميع أجزاء اسم الحزمة، مما يجعله "لا نهائيًا" في ما يتعلق بأجزاء الاستخدام.