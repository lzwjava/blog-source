...
أحاول الاتصال بجهاز في Hetzner Cloud باستخدام IPv6.  `ssh 2a01:4f8:c17:2000::/64` لا يعمل، لكن `ssh root@2a01:4f8:c17:2000::1` يعمل.
...
عند تشغيل `ssh root@192.168.1.3`، يظهر الإخراج التالي أن عميل SSH يطبق خيارات التكوين من ملف `~/.ssh/config`:
...
تظهر المشكلة عند استخدام عناوين IPv6 مع توجيه `ProxyCommand corkscrew localhost 7890 %h %p`، حيث قد لا يعالج أمر الوكيل هذا عناوين IPv6 بشكل صحيح.
