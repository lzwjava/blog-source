...
lang: ar
layout: post
title: هياكل البيانات المتقدمة في جافا
translated: true
type: note
---

تُعد هياكل البيانات أساس الخوارزميات الفعالة. دعنا نستكشف أربعة هياكل قوية: Skip List، وUnion-Find، وAVL Tree، وBinary Indexed Tree. تُستخدم هذه الهياكل على نطاق واسع في السيناريوهات التي تتطلب عمليات بحث، أو اتحاد، أو موازنة، أو استعلامات نطاق سريعة.

## 1. Skip List: البحث الاحتمالي
Skip List هي قائمة مرتبطة متعددة الطبقات تتيح البحث، والإدراج، والحذف السريع بتعقيد زمني متوسط O(log n)، مما يوفر بديلاً للأشجار المتوازنة.
...
## 2. Union-Find (Disjoint Set): تتبع الاتصال
يدير Union-Find المجموعات المنفصلة بكفاءة، ويدعم عمليات الاتحاد والبحث في وقت شبه ثابت O(1) مع ضغط المسار والاستدلالات الرتبية.
...
## 3. AVL Tree: شجرة البحث الثنائية ذاتية الموازنة
شجرة AVL هي شجرة بحث ثنائية ذاتية الموازنة حيث يكون فرق الارتفاع بين الأشجار الفرعية (عامل التوازن) على الأكثر 1، مما يضمن عمليات بـ O(log n).
...
```
**المخرجات:** `Preorder: 30 20 10 25 40 50`

## 4. Binary Indexed Tree (Fenwick Tree): استعلامات النطاق
تتعامل شجرة Binary Indexed Tree (BIT) بكفاءة مع استعلامات مجموع النطاق والتحديثات في وقت O(log n)، وغالبًا ما تُستخدم في البرمجة التنافسية.
...
# المدونة السابعة: خوارزميات البحث والمحاكاة في جافا

تتعامل خوارزميات البحث والمحاكاة مع إيجاد المسار والمشكلات الاحتمالية. دعنا نستكشف A* Search وMonte Carlo Simulation.

## 1. A* Search: إيجاد المسار الاستدلالي
A* هي خوارزمية بحث مستنيرة تستخدم دالة استدلالية لإيجاد أقصر مسار في رسم بياني، تجمع بين نقاط قوة Dijkstra والبحث الجشع. تُستخدم على نطاق واسع في الألعاب والملاحة.
...
## 2. Monte Carlo Simulation: التقدير الاحتمالي
تستخدم طرق Monte Carlo أخذ العينات العشوائية لتقدير النتائج، مثل تقريب π من خلال محاكاة النقاط في مربع ودائرة.
