...
هذا الكود ينشئ برنامج "مرحباً بالعالم". يستخدم مكالمات النظام لكتابة الرسالة إلى وحدة التحكم ثم الخروج. يحتوي قسم `.text` على التعليمات القابلة للتنفيذ، بدءًا من `_start`. يقوم أولاً بإعداد مكالمة `sys_write` (الطباعة إلى stdout) ثم مكالمة `sys_exit` (الخروج بشكل نظيف). يخزن قسم `.data` الرسالة نفسها ويحسب طولها.
