...
هذه الشفرة  تنشئ برنامجًا "مرحباً بالعالم".  يستخدم البرنامج  دعوات نظامية لكتابة الرسالة إلى وحدة التحكم ثم الخروج. يحتوي قسم `.text` على التعليمات القابلة للتنفيذ، بدءًا من `_start`. يقوم أولاً بإعداد  دعوة `sys_write` (الطباعة إلى stdout) ثم دعوة `sys_exit` (الخروج بشكل نظيف). يخزن قسم `.data` الرسالة نفسها ويحسب طولها.
