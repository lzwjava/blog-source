...
lang: ar
layout: post
title: ماسح الشبكة المحلية لعنوان IP
translated: true
---

## ماسح عنوان IP للشبكة المحلية (LAN)

يُعد هذا البرنامج النصي بلغة بايثون ماسحًا ضوئيًا للشبكة المحلية للبحث عن عناوين IP النشطة. يستخدم الأمر `ping` للتحقق من إمكانية الوصول إلى المضيف، ويستخدم تعدد الخيوط لتسريع عملية المسح. يحدد مؤشر إشارة (Semaphore) عدد الخيوط المتزامنة لتجنب إرهاق النظام. يأخذ البرنامج النصي عنوان الشبكة (مثل "192.168.1.0/24") كمدخل، ويطبع ما إذا كان كل عنوان IP في الشبكة قيد التشغيل أم متوقف.

يساعد هذا البرنامج النصي على تحديد الأجهزة على الشبكة، مثل جهاز توجيه شبكي من TP-LINK يعمل في وضع الجسر السلكي، من خلال البحث عن عناوين IP النشطة.
...
def is_host_up(host, port=None):
    """
    يتحقق مما إذا كان المضيف قيد التشغيل باستخدام ping أو telnet.
...
def scan_network(network, port=None):
    """
    يمسح شبكة بحثًا عن أجهزة مضيفة نشطة باستخدام الخيوط، مع تحديد عدد الخيوط المتزامنة.
    """
...
عادةً، يكون جهاز الكمبيوتر الخاص بي في 192.168.1.16.
...
ولكنه لا يعمل عند استخدام `192.168.1.0/28`، لأن 2^4 = 16، لذلك سيحاول من `192.168.1.0` إلى `192.168.1.15`، وهو ما لا يشمل `192.168.1.16`.
