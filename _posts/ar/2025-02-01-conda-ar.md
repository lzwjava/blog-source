...
انظر هذا الرابط: [https://www.reddit.com/r/MachineLearning/comments/myl8yn/d_is_anaconda_worth_the_trouble/](https://www.reddit.com/r/MachineLearning/comments/myl8yn/d_is_anaconda_worth_the_trouble/).

لقد واجهت مشاكل مع conda، وقد ناقشت الآخرون قضايا مشابهة.

مع مرور الوقت، يكون المرء غير مدرك للإعدادات المختلفة للبيئة. لذا، كنت مضطرًا إلى حذف الآخرين. الآن، أستخدم بشكل أساسي `Python 3.10.9` و بشكل نادر `Python 3.11.11`. لست بحاجة إلى العديد من البيئات. أنا بحاجة إلى واحدة أو اثنين فقط للتشغيل ما أنا في حاله البحث أو التعلم.

أفضل حلاً أبسط. إذا كان لدي مشروعان يتنافسان، فإني أفضّل استخدام واحد وازعاج عن تشغيل الآخر.

على سبيل المثال، مواجهتي للمشكلة التالية كانت مشكلة حقيقية:
...
رغم أن إعداد الخادم الوسيط لأجل conda بدا أنه يسارع الأشياء، لكنني قررت في النهاية إزالة الخادم الوسيط.
...
```
# >>> conda initialize >>>
# !! محتوى داخل هذه الكتلة يتم إدارتها بواسطة 'conda init' !!
...
بعد إعادة تشغيل iTerm2، قررت الانتقال إلى تثبيت Homebrew لـ Python. كان يبدو أن هناك تعدد من تثبيتات Python على كمبيوتر، وكنت بالفعل six، كما هو موضح أدناه.
...
حذف باستخدام الأمر، `brew uninstall python@3.11`.
