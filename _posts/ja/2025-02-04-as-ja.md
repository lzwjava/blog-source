...
lang: ja
layout: post
title: macOSでのアセンブリ：Hello World
translated: true
---
...
このコードは"Hello, World!"プログラムを作成します。コンソールにメッセージを書き込み、終了するためにシステムコールを使用します。`.text`セクションには、`_start`から始まる実行可能命令が格納されています。まず`sys_write`コール（stdoutへの出力）を設定し、次に`sys_exit`コール（クリーンな終了）を設定します。`.data`セクションにはメッセージ自体が格納され、その長さが計算されます。
