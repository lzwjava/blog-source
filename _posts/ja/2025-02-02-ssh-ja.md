...
`git push`操作が遅く、`ssh git@github.com`フェーズで遅延が発生しました。これを解決するために、SSHでプロキシを使用するように設定しました。

`corkscrew`をSSHトンネリングに使用するには、最初にインストールする必要があります。macOSでは、`brew install corkscrew`コマンドでHomebrewを使用できます。
