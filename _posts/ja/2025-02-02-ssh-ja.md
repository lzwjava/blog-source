...
`git push`操作が遅く、`ssh git@github.com`フェーズで遅延が発生していました。これを解決するために、SSHでプロキシを使用するように設定しました。

`corkscrew` をSSHトンネリングに使用するには、最初にインストールする必要があります。macOSでは、`brew install corkscrew`コマンドを使用してHomebrewでインストールできます。
...
## Outlineクライアントの依存関係のインストール

Outlineクライアントを実行するには、`fuse`が必要です。以下のコマンドを使用してインストールします。
