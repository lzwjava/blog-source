...
lang: ja
layout: post
title: グラフアルゴリズムのJava
translated: true
---

グラフは関係をモデル化し、そのアルゴリズムは最短経路や接続性の問題を解決します。DFS、BFS、ダイクストラのアルゴリズム、そしてクラスカルのアルゴリズムについて説明します。

## 1. DFS: 深さ優先探索
DFSは各分岐をできるだけ深く探索し、その後戻ることで探索を行います。
...
## 3. ダイクストラのアルゴリズム: 最短経路
ダイクストラのアルゴリズムは重み付きグラフでの最短経路を見つけます。
...
```
**出力:** `Distances from 0: [0, 3, 1, 8]`

## 4. クラスカルのアルゴリズム: 最小全域木
クラスカルのアルゴリズムはエッジをソートし、貪欲に追加して最小全域木を構築します。
