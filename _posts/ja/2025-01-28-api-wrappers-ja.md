---
audio: true
lang: ja
layout: post
title: 原則としてラッパーではなく、生のHTTPリクエストを使用してください。
translated: true
---

python
import requests
import json
import time

def translate_text(text, target_language, special=False):
    if not text or not text.strip():
        return ""
    if target_language == 'en':
        print(f"  English translation skipped: {text[:50]}...")
        return text
    print(f"  Translating text: {text[:50]}...")

    retries = 3
    for attempt in range(retries):
        try:
            response = client.chat.completions.create(
                model=MODEL_NAME,
                messages=[
                    {"role": "system", "content": create_translation_prompt(target_language, special)},
                    {"role": "user", "content": text}
                ],
                stream=False
            )
            if not response or not response.choices or not response.choices[0].message.content:
                print(f"  Error: Translation response is empty or invalid: {response}")
            if response and response.choices:
                translated_text = response.choices[0].message.content
                return translated_text
            else:
                print(f"  Translation failed on attempt {attempt + 1}.")
                if attempt == retries - 1:
                    return None
        except Exception as e:
            print(f"  Translation failed with error on attempt {attempt + 1}: {e}")
            if attempt == retries - 1:
                return None
            time.sleep(1)  # Wait before retrying
    return None


エラー:

bash
 Translation failed with error on attempt 1: Expecting value: line 5 column 1 (char 4)


このエラーは、DeepSeek APIが有効なJSONではないレスポンス（HTMLや他の形式）を返していることを示しています。APIはJSONを返すことが期待されているため、これは予期しない動作です。問題は一時的なAPIの問題、レート制限、プロンプトの問題などが原因である可能性があります。これを適切に処理するために、エラーをログに記録し、必要に応じて再試行することが重要です。