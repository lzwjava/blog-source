    ...
    try:
        ...
        if response_json and response_json['choices']:
            ...
            if 'message' in choice and 'content' in choice['message']:
                ...
                return tool_calls # 処理用のツール呼び出しを返す
            else:
...
def call_codestral_api(prompt, model="codestral-latest"):
    ...
    try:
        ...
        if response_json and response_json['choices']:
            ...
            print(f"Codestral APIエラー: 無効なレスポンス形式: {response_json}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"Codestral APIエラー: {e}")
        if e.response:
            print(f"レスポンスステータスコード: {e.response.status_code}")
            print(f"レスポンス内容: {e.response.text}")
        return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MistralおよびCodestral APIをテストする。")
    parser.add_argument("--type", type=str, default="mistral", choices=["mistral", "codestral"], help="呼び出すAPIのタイプ（mistralまたはcodestral）")
