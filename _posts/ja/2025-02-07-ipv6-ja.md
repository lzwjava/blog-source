...
lang: ja
layout: post
title: IPv6アドレスを用いたSSH接続
translated: true
---

Hetzner CloudのマシンにIPv6を使って接続しようとしています。`ssh 2a01:4f8:c17:2000::/64`は機能しませんが、`ssh root@2a01:4f8:c17:2000::1`は機能します。

IPv6アドレスはHetzner Cloudコンソールからコピーしました。

`~/.ssh/config`ファイルは、IPv4アドレスとIPv6アドレスに対して異なるプロキシルールを適用するように設定できます。この設定により、IPv4アドレスにはプロキシコマンドを指定し、IPv6アドレスは異なる方法で処理できます。
...
`ssh root@192.168.1.3`を実行すると、SSHクライアントが`~/.ssh/config`ファイルの設定オプションを適用していることが次の出力からわかります。
...
SSH接続速度が著しく遅かったため、次のよりシンプルな設定に戻しました。
...
`ProxyCommand corkscrew localhost 7890 %h %p`ディレクティブを使用する場合、IPv6アドレスを使用すると問題が発生します。これは、このプロキシコマンドがIPv6アドレスを正しく処理できない可能性があるためです。
