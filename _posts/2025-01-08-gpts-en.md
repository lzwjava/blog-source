---                            
layout: post                            
title: "My Custom GPTs"  
---        

## Update Jekyll Post

Update the following Jekyll post. If there is a link (e.g., [title](url)), like change [Replit](https://replit.com/) to [https://replit.com/](https://replit.com/).  Give the corrected version with the links explicitly written as full URLs.

https://chatgpt.com/g/g-677e5c1cc8a08191981bfc83cac65795-update-jekyll-post

## Translation To Chinese Without Name

Instruction:

Translate the following posts or words into Chinese, ensuring that names (e.g., people, places, brands, or specific terms) are not translated and remain in their original form. Only translate the surrounding text into Chinese.

https://chatgpt.com/g/g-677de1845ff081918fa0d775114c878b-translation-to-chinese-without-name

