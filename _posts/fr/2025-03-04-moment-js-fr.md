...
Je vais vous montrer comment utiliser les bibliothèques `moment` et `moment-timezone` en JavaScript. Ce sont des bibliothèques populaires pour la manipulation des dates et des heures.

1. Tout d'abord, installez les packages si ce n'est pas déjà fait :
```bash
...
const now = moment();
console.log(now.format('YYYY-MM-DD HH:mm:ss')); // par exemple, "2025-03-03 14:30:45"
...
console.log(tomorrow.format('ddd, MMM D')); // par exemple, "Tue, Mar 4"
...
console.log(timezones); // Tableau de noms de fuseaux horaires
```
...
- `HH` : heure (24 heures) (14)
- `hh` : heure (12 heures) (02)
- `mm` : minutes (30)
- `ss` : secondes (45)
...
// Opérations de fuseau horaire
momentTimezone.tz('2025-03-03', 'America/Los_Angeles');
momentTimezone.tz.guess(); // Deviner le fuseau horaire de l'utilisateur
```

Notes :
1. Ces versions (^2.14.1 et ^0.5.7) sont assez anciennes (d'environ 2016). Pensez à utiliser les dernières versions sauf si vous avez des exigences de compatibilité spécifiques.
2. Moment.js est maintenant considéré comme un projet hérité en mode de maintenance. Pour de nouveaux projets, vous pourriez vouloir envisager des alternatives comme `date-fns` ou `luxon`.