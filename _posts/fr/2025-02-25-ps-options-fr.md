...
lang: fr
layout: post
title: Comprendre la commande `ps` et ses options
translated: true
---

Me laissez décomposer la commande `ps -ef | grep java`, puis expliquer certaines options courantes pour la commande `ps` afin de vous aider à comprendre son fonctionnement et comment vous pouvez la personnaliser.
...
- **`grep java`** : Cela filtre la sortie pour afficher uniquement les lignes contenant le mot "java", facilitant ainsi la recherche des processus liés à Java (comme une application ou un serveur Java).
...
- **`--forest`** : Affiche les processus sous forme d'arbre, mettant en évidence les relations parent-enfant entre les processus, ce qui est utile pour comprendre les hiérarchies de processus.
...
- **`aux`** : Une combinaison populaire au style BSD qui affiche tous les processus (`a`), avec des détails orientés utilisateur (`u`), et inclut les processus sans terminal de contrôle (`x`).
...
- `ps aux` (notez : pas de tiret ici, style BSD) est une manière courante de voir tous les processus avec des détails utilisateur.
- `ps --forest -e` affiche tous les processus sous forme d'arbre.
