...
1. **Configurer `tlmgr` en mode utilisateur correctement** :
   ...
   - Vous pouvez également initialiser le répertoire spécifique à l'utilisateur de TeX Live avant d'utiliser `tlmgr`.
...
2. **Initialiser `tlmgr` en mode utilisateur** :
   ...
   Cela créera l'arborescence spécifique à l'utilisateur de TeX Live dans `~/texmf`. Une fois initialisé, vous pouvez ensuite exécuter les commandes `tlmgr update` et `install`.
...
Si vous exécutez cela dans le cadre d'un workflow GitHub Actions plus large, vous pouvez intégrer ces modifications dans la section du job concernée :
...
- `tlmgr init-usertree` : Initialise l'arborescence spécifique à l'utilisateur de TeX Live (généralement située dans `~/texmf`).
- `tlmgr update --self` : Met à jour le package `tlmgr` lui-même vers la dernière version.
...
1. **Supprimer l'installation système de TeX Live :**
   - Évitez d'utiliser `apt-get install texlive-xetex` car cela fournit une installation partielle de TeX Live avec une fonctionnalité `tlmgr` limitée.
...
2. **Configuration de Make :**
   - Installe `make` en utilisant `apt-get`, garantissant qu'il est disponible pour la construction de votre CV.
...
- **Temps d'installation de TeX Live :**
  - **Mise en cache :** L'inclusion de l'étape `actions/cache@v3` aide à atténuer le temps d'installation long en mettant en cache TeX Live après la première exécution. Assurez-vous que la clé de cache (`${{ runner.os }}-texlive-2023`) reflète avec précision la version de TeX Live pour éviter les incompatibilités de cache.
...
1. **Problèmes de cache manquant :**
   - Assurez-vous que la clé de cache reflète avec précision la version de TeX Live. Si vous mettez à jour TeX Live ou modifiez le chemin d'installation, mettez à jour la clé de cache en conséquence.
...
4. **Problèmes de PATH :**
   - Assurez-vous que le PATH est correctement mis à jour. Vous pouvez ajouter une étape pour imprimer le PATH à des fins de débogage :
     ```yaml
     - name: Print PATH
       run: echo $PATH
     ```
...
Voici un workflow plus complet qui inclut des étapes de vérification pour s'assurer que tout est correctement configuré :
