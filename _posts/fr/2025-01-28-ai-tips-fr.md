...
* Il existe encore des cas où vous devez utiliser Visual Studio Code, par exemple pour les scénarios d'éditeur de fusion git, où j'utilise toujours `git config --global core.editor "code --wait"`.
...
* Une raison pour laquelle nous n'utilisons pas Postman ou Figma à l'ère de l'IA est que leurs fonctionnalités ne peuvent pas être générées par le texte. Ils manquent également d'un raccourci clavier Commande + K pour déclencher le remplacement de composant.
...
* Pourquoi mettre à niveau Figma pour qu'il soit alimenté par l'IA pour créer des interfaces utilisateur alors que nous pouvons directement utiliser le code pour générer des interfaces utilisateur, car ce dernier sera alimenté par l'IA ?
