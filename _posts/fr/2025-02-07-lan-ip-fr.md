...
Ce script permet d'identifier les périphériques sur le réseau, tels qu'un routeur maillé TP-LINK fonctionnant en mode pont filaire, en recherchant les adresses IP actives.
...
def is_host_up(host, port=None):
    ...
    Si le port est spécifié, utilise telnet pour vérifier si le port est ouvert.
    Sinon, utilise ping.
...
def scan_ip(ip_str, up_ips, port=None):
    """
    Analyse une seule adresse IP et affiche son état.
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Analyser un réseau pour les hôtes actifs.")
...
Habituellement, ma machine est à l'adresse 192.168.1.16.

Donc cela fonctionne avec la commande ci-dessous.
