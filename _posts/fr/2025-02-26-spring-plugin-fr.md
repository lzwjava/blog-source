...
### **1. Vérifier que votre projet est un projet Spring Boot**
...
- **Étendre `spring-boot-starter-parent` (Recommandé)** :
  - Dans votre `pom.xml`, définissez `spring-boot-starter-parent` comme parent pour gérer les dépendances Spring Boot et les versions des plugins.
  - Exemple :
    ```xml
    <parent>
        ...
        <relativePath/> <!-- Chercher le parent depuis le repository -->
    </parent>
    ```

- **Alternativement, Utiliser le BOM `spring-boot-dependencies` (Bill of Materials)** :
  - Si vous ne pouvez pas utiliser `spring-boot-starter-parent`, importez le BOM `spring-boot-dependencies` dans la section `dependencyManagement`.
  - Exemple :
    ```xml
...
### **2. Ajouter le `spring-boot-maven-plugin` à votre `pom.xml`**
...
- **Si vous n'utilisez pas `spring-boot-starter-parent`** :
  - Spécifiez explicitement la version, correspondant à la version Spring Boot utilisée.
  - Exemple :
    ```xml
    <build>
        <plugins>
...
### **3. Utiliser les objectifs (goals) du plugin**
...
- **`spring-boot:run`**
  - Exécute votre application Spring Boot directement depuis Maven en utilisant un serveur web embarqué (par exemple, Tomcat).
...
- **`spring-boot:build-info`**
  - Génère un fichier `build-info.properties` contenant des informations de build (par exemple, l'heure de build, la version).
  - Ces informations sont accessibles dans votre application en utilisant le bean `BuildProperties` de Spring Boot ou les annotations `@Value`.
  - Commande :
    ```
...
### **4. Personnaliser la configuration du plugin (Optionnel)**
...
- **Exclure des dépendances du Fat JAR** :
...
- **Définir les arguments de l'application** :
...
- **Construire des fichiers WAR** :
  - Si vous construisez un fichier WAR pour le déployer dans un conteneur de servlets externe, assurez-vous que le packaging de votre projet est défini sur `war` dans `pom.xml` :
    ```xml
    <packaging>war</packaging>
    ```
  - Vous devrez peut-être aussi exclure les conteneurs de servlets embarqués (par exemple, Tomcat) s'ils sont fournis par l'environnement. Ajoutez la dépendance suivante avec le scope `provided` :
    ```xml
    <dependencies>
        <dependency>
            <groupId>org.springframework.boot</groupId>
...
- **Comportement par défaut** :
  ...
  - L'artefact reconditionné est l'artefact principal, et le JAR/WAR original est attaché avec un classifieur (par exemple, `original`).
...
1. Assurez-vous que votre projet est un projet Spring Boot, généralement en étendant `spring-boot-starter-parent` ou en utilisant le BOM `spring-boot-dependencies`.
