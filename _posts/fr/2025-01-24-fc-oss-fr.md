...
def handler(event, context):
    ...
    try:
        # Utilisez l'ARN du rôle fourni pour créer l'objet d'authentification
        ...
        # 2. Initialiser le client OSS avec le bon point de terminaison
        bucket = oss2.Bucket(
            sts_auth,
            oss_endpoint,
            oss_bucket_name
        ...
        # 3. Téléchargement et téléversement en flux
        logging.info("Début du téléchargement...")
        with requests.get(model_url, stream=True) as response:
            response.raise_for_status()
            
            # Téléversement vers OSS
        ...
        if result.status == 200:
            logging.info("Téléversement réussi !")
            return "Success"
        else:
            raise Exception(f"Échec du téléversement OSS : {result.status}")
