...
J'ai rencontré des problèmes avec conda, et d'autres en ont discuté également.

Avec le temps, vous perdez de vue les différents paramètres de l'environnement. Donc, j'ai dû les supprimer. Maintenant, j'utilise principalement `Python 3.10.9` et occasionnellement `Python 3.11.11`. Cependant, j'utilise rarement même `Python 3.11.11`. Personnellement, je n'ai pas besoin de trop d'environnements. J'en ai besoin d'un ou deux pour faire fonctionner ce que je recherche ou apprends actuellement.

Je préfère une solution plus simple. Si j'ai deux projets qui sont en conflit l'un avec l'autre, je préfère utiliser l'un et renoncer à exécuter l'autre.

Par exemple, rencontrer le problème suivant est vraiment ennuyeux :
...
Bien que la configuration d'un proxy pour conda ait semblé accélérer les choses au début, j'ai finalement décidé de le supprimer.
...
Après avoir redémarré iTerm2, j'ai décidé de passer à l'installation de Python via Homebrew. Il semblait y avoir plusieurs installations de Python sur mon ordinateur, et j'en avais déjà six, comme indiqué ci-dessous.
