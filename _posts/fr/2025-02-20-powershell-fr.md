...
Pour les scripts PowerShell, l'extension de fichier conventionnelle est `.ps1`. Voici comment vous listeriez ces commandes dans un fichier `.ps1` :
...
Veuillez noter que vous devrez peut-être ajuster votre politique d'exécution pour exécuter des scripts PowerShell :

```powershell
Set-ExecutionPolicy RemoteSigned -Scope CurrentUser
```

Cette commande permet l'exécution des scripts, mais n'oubliez pas de la rétablir ou de l'utiliser avec prudence car elle affecte les paramètres de sécurité du système.