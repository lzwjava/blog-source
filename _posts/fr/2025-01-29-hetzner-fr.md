...
Un serveur à Helsinki avec une configuration de 2 VCPUs AMD, 2 Go de RAM, 40 Go SSD et 20 To de trafic coûte 4,49 USD par mois.
...
Il est intéressant de noter qu'ils ne suivent pas les tendances pour choisir des emplacements populaires. Leurs emplacements diffèrent de ceux de Vultr ou Digital Ocean.

Les paramètres du pare-feu sont faciles à utiliser. Bien que ce soit la première fois que j'utilise ce service, j'ai rapidement configuré les paramètres corrects pour mon serveur proxy.

> sudo bash -c "$(wget -qO- https://raw.githubusercontent.com/Jigsaw-Code/outline-server/master/src/server_manager/install_scripts/install_server.sh)"

La vitesse du serveur Hetzner à Helsinki est très rapide. Avec l'application Speedtest iOS, la vitesse de téléchargement est de 423 Mbps et la vitesse de téléchargement est de 56,1 Mbps.

Le ping dans Shadowrocket est de 1175 ms, mais ce n'est pas un problème majeur.
...
# Obtenir le jeton API à partir de la variable d'environnement
...
for server in servers:
    ...
    print(f"État du serveur : {server.status}")
...
print(f"État du serveur spécifique : {server.status}")
