...
lang: fr
layout: post
title: Journal de la Mise à Jour de Brew
translated: true
type: note
---
...
==> Notes
Les complétions zsh ont été installées dans :
...
car macOS fournit un logiciel similaire et installer ce logiciel en
...
car macOS fournit déjà ce logiciel et installer une autre version en
parallèle peut causer toutes sortes de problèmes.

Si vous devez avoir ruby en premier dans votre PATH, exécutez :
...
==> perl
Par défaut, les modules cpan non brassés sont installés dans la Cellar. Si vous souhaitez
que vos modules persistent entre les mises à jour, nous recommandons d'utiliser `local::lib`.

Vous pouvez le configurer comme ceci :
  PERL_MM_OPT="INSTALL_BASE=$HOME/perl5" cpan local::lib
Et ajoutez ce qui suit à votre profil de shell, par exemple ~/.profile ou ~/.zshrc
  eval "$(perl -I$HOME/perl5/lib/perl5 -Mlocal::lib=$HOME/perl5)"
==> awscli
Le répertoire "examples" a été installé dans :
  /opt/homebrew/share/awscli/examples

Les complétions et fonctions zsh ont été installées dans :
  /opt/homebrew/share/zsh/site-functions
==> php
...
Pour démarrer php maintenant et le redémarrer à la connexion :
...
nginx puisse s'exécuter sans sudo.
...
Mettez à jour votre configuration git pour terminer l'installation :
  ...
  # Mettre à jour la configuration git globale
  $ git lfs install

  # Mettre à jour la configuration git système
...
(comportement par défaut de macOS), installez ChmodBPF :
