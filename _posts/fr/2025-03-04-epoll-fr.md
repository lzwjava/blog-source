...
lang: fr
layout: post
title: Utilisation de epoll sous Linux
translated: true
---

# Comment utiliser `epoll` sous Linux

`epoll` est un appel système Linux conçu pour surveiller efficacement plusieurs descripteurs de fichiers pour des événements, tels que la disponibilité de données à lire ou la préparation d'un socket à écrire. Il est particulièrement utile en programmation réseau, où un serveur pourrait avoir besoin de gérer de nombreuses connexions simultanément. Par rapport à des mécanismes plus anciens comme `select` ou `poll`, `epoll` s'adapte mieux à un grand nombre de descripteurs de fichiers car il évite de scanner à nouveau tous les descripteurs pour la préparation.
...
- **Valeur de retour** : Nombre de descripteurs de fichiers avec des événements (`nfds`), ou `-1` en cas d'erreur.
...
- **Sockets clients** : Surveille également `EPOLLIN` pour détecter les données entrantes.
- **Simplification** : Suppose que `write` se termine complètement. Dans une application réelle, mettez en tampon les données et utilisez `EPOLLOUT` pour les écritures partielles.
...
Pour les applications simples, le mode déclencheur par niveau est recommandé. Pour des besoins de haute performance, envisagez le mode déclencheur par bord avec une gestion minutieuse de toutes les données disponibles. Associez toujours `epoll` à une E/S non bloquante pour obtenir les meilleurs résultats.