...
- Le fichier `butterknife-6.1.0.jar` téléchargé. Vous pouvez généralement trouver des versions plus anciennes dans des dépôts comme Maven Central ou via des sources archivées si vous avez le fichier `.jar` localement.
...
1. **Localiser le dossier `libs`** :
   - Dans votre projet Android Studio, accédez au module `app`.
...
2. **Mise en page XML** :
   - Assurez-vous que votre fichier de mise en page (par exemple, `res/layout/activity_main.xml`) contient les vues avec les IDs correspondants :
     ```xml
...
3. **Lier les vues** :
   - L'appel `ButterKnife.inject(this)` dans `onCreate` lie les champs annotés aux vues de la mise en page. Notez que dans la version 6.1.0, `inject` est utilisé au lieu de `bind` (qui a été introduit dans les versions ultérieures comme 7.x et 8.x).
...
- **Statut obsolète** : Butter Knife n'est plus maintenu (le développement s'est arrêté vers 2020), et la recommandation officielle est d'utiliser la liaison de vues ou Jetpack Compose pour les nouveaux projets.
- **Configuration alternative** : Si vous utilisiez une version plus récente (par exemple, 10.2.3), vous devriez ajouter une dépendance de processeur d'annotations (par exemple, `annotationProcessor 'com.jakewharton:butterknife-compiler:10.2.3'`), mais pour 6.1.0 avec un fichier `.jar`, ce n'est pas nécessaire.
- **Dépannage** :
...
### Recommandation
Bien que ce guide vous montre comment utiliser `butterknife-6.1.0.jar`, envisagez de migrer vers la liaison de vues pour une approche plus moderne et prise en charge. Voici une comparaison rapide :
- **Butter Knife 6.1.0** : Utilise `@InjectView`, réflexion en temps d'exécution, intégration manuelle du fichier `.jar`.
- **Liaison de vues** : Intégrée à Android, aucune dépendance externe, génère des classes de liaison en temps de compilation, et est sécurisée contre les nullités.