...
Pour utiliser un pilote JDBC (Java Database Connectivity) en Java, vous devez suivre une série d'étapes pour établir une connexion à une base de données, exécuter des requêtes et traiter les résultats. Ci-dessous, je vais vous guider à travers le processus étape par étape de manière claire et pratique.
...
### Étape 4 : Créer un Statement et exécuter des requêtes
...
Fermez toujours vos objets `ResultSet`, `Statement` et `Connection` pour libérer les ressources. L'utilisation du try-with-resources (comme montré ci-dessus) gère cela automatiquement pour vous.

### Exemple complet fonctionnel (MySQL)
Voici un exemple complet supposant une table `users` avec des colonnes `id` (int) et `name` (varchar) :
...
- **Sécurité** : Utilisez `PreparedStatement` pour les requêtes avec entrées utilisateur pour éviter les injections SQL.
- **Pool de connexions** : Pour les applications en production, envisagez d'utiliser un pool de connexions (par exemple, HikariCP) au lieu de `DriverManager`.
