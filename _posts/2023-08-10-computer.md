...
1. We want to make a computer to do machine learning
2. We buy the latest and most affordable components in 2023 to build
3. The computer couldn't start up. We asked an expert for help two times. One is that the motherboard power supply connector is loose. The other reason is that the front panel start button connector is loose.
4. We don't need to buy vertical GPU brackets in our case. We should carefully analyze the reason instead of buying extra components for a quick solution.
5. We should make cables place on the other side of the computer case. So the cables will not cover the motherboard and we can see it clearly.
...
We ask an expert for help two times. One is that the motherboard power supply connector is loose. The other reason is that the front panel start button connector is loose.

When we ask the specialists to help us, we need to do it ourselves to learn. They should only guide us. Otherwise, we will easily forget and don't know what to do when facing problems next time. 
