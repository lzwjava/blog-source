...
這段 Python 腳本掃描本地網絡以尋找活躍的 IP 地址。它使用 `ping` 命令檢查主機是否可達，並使用多線程來加快掃描過程。信號量限制同時運行的線程數量，以避免壓垮系統。腳本接收網絡地址（例如，「192.168.1.0/24」）作為輸入，並列印網絡中每個 IP 地址的狀態（啟動或關閉）。
...
def is_host_up(host, port=None):
    """
    使用 ping 或 telnet 檢查主機是否已啟動。
    如果指定 port，則使用 telnet 檢查端口是否開啟。
    否則，使用 ping。
    如果主機已啟動，則返回 True，否則返回 False。
    """
    if port:
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="掃描網絡以尋找活躍的主機。")
...
腳本識別活躍的 IP 地址。為確保正確的網絡通訊，請驗證代理設置是否已配置為跳過這些本地 IP。
