...
我發現 `git push` 操作速度很慢，延遲發生在 `ssh git@github.com` 階段。為了解決此問題，我設定 SSH 使用 Proxy。

要使用 `corkscrew` 進行 SSH 通道建立，您首先需要安裝它。在 macOS 上，您可以使用 Homebrew，指令如下：`brew install corkscrew`。
...
## 安裝 Outline 客戶端所需的相依套件

Outline 客戶端需要 `fuse` 才能執行。請使用以下指令安裝：
