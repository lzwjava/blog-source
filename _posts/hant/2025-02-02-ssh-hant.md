...
我發現 `git push` 操作速度很慢，延遲發生在 `ssh git@github.com` 階段。為了解決此問題，我將 SSH 設定為使用 Proxy。

要使用 `corkscrew` 進行 SSH 通道連線，您首先需要安裝它。在 macOS 上，您可以使用 Homebrew 並執行以下命令：`brew install corkscrew`。
