...
lang: hant
layout: post
title: macOS 組合語言入門：Hello World
translated: true
type: note
---
...
這段程式碼建立了一個「Hello, World!」程式。它使用系統呼叫將訊息寫入控制台，然後退出。`.text` 區段包含可執行指令，從 `_start` 開始。它首先設置 `sys_write` 呼叫（輸出到 stdout），然後是 `sys_exit` 呼叫（乾淨退出）。`.data` 區段儲存訊息本身並計算其長度。
