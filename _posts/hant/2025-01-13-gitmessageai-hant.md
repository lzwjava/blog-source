...
lang: hant
layout: post
title: AI 驅動的 Git 提交訊息
translated: true
---

這個 Python 腳本應該放置在系統 PATH 中的目錄中，例如 `~/bin`。
...
def call_gemini_api(prompt):
    ...
    try:
        ...
        response.raise_for_status()  # 對於狀態碼錯誤拋出異常
        response_json = response.json()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="使用 AI 生成提交信息並提交更改。")
    ...
    parser.add_argument('--api', type=str, default='deepseek', choices=['deepseek', 'gemini', 'mistral'], help='用於提交信息生成的 API (deepseek, gemini, 或 mistral)。')
    args = parser.parse_args()
    gitmessageai(push=args.push, only_message=args.only_message, api=args.api)


這個腳本可以用不同的 API 調用。例如：
...
* 一個是僅發送檔案名更改，而不是使用 `git diff` 讀取檔案的詳細更改。我們不想給 AI 服務 API 提供太多詳細信息。在這種情況下，我們不需要它，因為很少有人會仔細閱讀提交信息。
