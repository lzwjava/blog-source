...
21:32:14.216308 exec-cmd.c:139          trace: aufgelöster Pfad zur ausführbaren Datei vom Darwin-Stack: /Applications/Xcode.app/Contents/Developer/usr/bin/git
21:32:14.216594 exec-cmd.c:238          trace: aufgelöstes Verzeichnis der ausführbaren Datei: /Applications/Xcode.app/Contents/Developer/usr/bin
21:32:14.216949 git.c:460               trace: eingebaut: git push
21:32:14.218655 run-command.c:655       trace: run_command: GIT_PREFIX wird aufgehoben; ssh git@github.com 'git-receive-pack '\''lzwjava/lzwjava.github.io.git'\'''
Everything up-to-date
```

Ich hatte langsame `git push`-Operationen mit Verzögerungen während der Phase `ssh git@github.com`. Um dies zu beheben, habe ich SSH so konfiguriert, dass es einen Proxy verwendet.

Um `corkscrew` für SSH-Tunneling zu verwenden, müssen Sie es zuerst installieren. Unter macOS können Sie Homebrew mit dem Befehl verwenden: `brew install corkscrew`.
...
## Abhängigkeiten für Outline Client installieren

Der Outline Client benötigt `fuse`, um zu laufen. Installieren Sie es mit den folgenden Befehlen:
