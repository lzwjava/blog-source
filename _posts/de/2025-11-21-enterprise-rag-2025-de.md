...
title: Enterprise RAG Stack für 2025
translated: true
type: note
---

Hier ist ein vollständiges, produktionsreifes Minimalbeispiel, das **genau das abbildet, was 60-70 % der Unternehmen im Jahr 2025** für internen RAG über Confluence + Jira + GitHub einsetzen, unter Verwendung von:

- LlamaIndex (v0.11+)
- Azure OpenAI (gpt-4o + text-embedding-3-large)
- Pinecone (serverless oder pod)
- Unstructured.io für Confluence/Jira-Parsing
- Metadatenfilterung + Hybrid Search + Reranking
...
PINECONE_API_KEY=...
COHERE_API_KEY=...  # für Reranker (optional, aber dringend empfohlen)
GITHUB_TOKEN=ghp_...
```

### 3. Ingestion-Skript (täglich oder per Webhook ausführen)
```python
# ingest.py
import os
...
# ==== Pinecone-Setup ====
pinecone.init(api_key=os.getenv("PINECONE_API_KEY"), environment="gcp-starter")  # oder serverless
...
if index_name not in pinecone.list_indexes():
    pinecone.create_index(
        ...
        metric="cosine",
        spec=pinecone.ServerlessSpec(cloud="aws", region="us-east-1")  # oder PodSpec
    )
pinecone_index = pinecone.Index(index_name)
vector_store = PineconeVectorStore(pinecone_index=pinecone_index, namespace="prod")

# ==== Loader mit umfangreichen Metadaten ====
documents = []
...
# GitHub (nur master branch, oder nach Pfad filtern)
...
print(f"Indexierte {len(nodes)} Nodes")
```

### 4. Query Engine (FastAPI oder Streamlit oder Slack Bot)
```python
# query.py
...
# Gleiche Einstellungen wie oben (embed_model, llm)
...
# ==== Query Engine mit Filterung + Reranking ====
...
query_engine = RetrieverQueryEngine(
    ...
    node_postprocessors=[
        ...
        CohereRerank(cohere_api_key=os.getenv("COHERE_API_KEY"), top_n=5),  # magischer Genauigkeits-Boost
    ],
)

# ==== Beispielabfragen mit Metadatenfiltern ====
def ask(question: str, project: str = None, source: str = None):
    ...
    print("Antwort:", response.response)
    print("\nQuellen:")
...
# Beispiele
ask("Wie deployen wir den Payment-Service in die Produktion?")
ask("Was ist der aktuelle Status des Fraud-Detection-Epics?", project="AI")
ask("Zeige mir das aktuelle Architekturdiagramm für den Auth-Service", source="confluence")
```

Dieses exakte Muster (oder sehr ähnlich) läuft Ende 2025 bei >10.000 Unternehmen. Sie erhalten:

- Vollständige Zitate mit klickbaren URLs
- Metadatenfilterung (Team, Projekt, Status, Repository)
- Hybrid Search + Reranking → nahezu Google-ähnliche Relevanz bei internen Dokumenten
- Sicher (gesamter Azure/Pinecone-Datenverkehr bleibt in Ihrer VPC bei Verwendung privater Endpunkte)

Führen Sie einfach `python ingest.py` nächtlich aus (oder verwenden Sie Airflow/Kubeflow/Dagster), exponieren Sie `ask()` über FastAPI oder einen Slack-Bot, und Sie haben Ihren eigenen internen Grok/ChatGPT.