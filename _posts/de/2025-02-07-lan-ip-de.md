...
Dieses Python-Skript scannt ein lokales Netzwerk nach aktiven IP-Adressen. Es verwendet den Befehl `ping`, um zu überprüfen, ob ein Host erreichbar ist, und setzt Multithreading ein, um den Scanvorgang zu beschleunigen. Ein Semaphore begrenzt die Anzahl gleichzeitiger Threads, um eine Überlastung des Systems zu vermeiden. Das Skript nimmt eine Netzwerkadresse (z. B. "192.168.1.0/24") als Eingabe entgegen und gibt aus, ob jede IP-Adresse im Netzwerk aktiv oder inaktiv ist.
...
Normalerweise befindet sich mein Rechner unter 192.168.1.16.
...
da 32 - 27 = 5, 2^5 = 32, also werden `192.168.1.0` bis `192.168.1.31` versucht.

Es funktioniert jedoch nicht mit `192.168.1.0/28`, da 2^4 = 16, also werden `192.168.1.0` bis `192.168.1.15` versucht, was `192.168.1.16` nicht abdeckt.
