...
Este script ayuda a identificar dispositivos en la red, como un router de malla TP-LINK que funciona en modo puente cableado, mediante el escaneo de direcciones IP activas.
...
Normalmente, mi máquina está en 192.168.1.16.

Así que funciona usando el comando de abajo.

```bash
python scripts/ip_scan.py 192.168.1.0/27 -p 22
```

porque 32 - 27 = 5, 2^5 = 32, así que intentará `192.168.1.0` a `192.168.1.31`.

Pero no funciona cuando se usa `192.168.1.0/28`, porque 2^4 = 16, así que intentará `192.168.1.0` a `192.168.1.15`, lo que no cubre `192.168.1.16`.
