...
He experimentado operaciones `git push` lentas, con retrasos que ocurren durante la fase `ssh git@github.com`. Para abordar esto, configuré SSH para usar un proxy.

Para usar `corkscrew` para el tunelamiento SSH, primero necesitarás instalarlo. En macOS, puedes usar Homebrew con el comando: `brew install corkscrew`.
