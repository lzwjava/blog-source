...
Experimenté operaciones `git push` lentas, con retrasos que ocurrían durante la fase `ssh git@github.com`. Para solucionar esto, configuré SSH para usar un proxy.

Para usar `corkscrew` para el tunelado SSH, primero necesitarás instalarlo. En macOS, puedes usar Homebrew con el comando: `brew install corkscrew`.
...
Autenticado en 138.201.174.0 (vía proxy) usando "publickey".
...
## Instalar Dependencias para el Cliente Outline

El Cliente Outline requiere `fuse` para ejecutarse. Instálalo usando los siguientes comandos:
