...
lang: es
layout: post
title: Modelos Instruct frente a Modelos de Código Comparados
translated: true
type: note
---
...
* **Desventajas en comparación con los modelos Base**
  Un modelo base (sin ajuste por instrucciones) es como un estudiante culto pero sin enfoque: fuerte en comprensión del lenguaje, pero carece de especificidad para tareas o adherencia a tus direcciones ([Medium][1]).
...
* **Ejemplos y Capacidades**
  ...
  * **Variantes Code Llama – Instruct**: Estos son modelos centrados en código que también siguen instrucciones, ofreciendo un gran rendimiento en benchmarks como HumanEval y MBPP ([arXiv][8]).
  * **DeepSeek Coder**: Ofrece versiones tanto Base como Instruct, entrenadas con cantidades masivas de código y con soporte de contexto largo (hasta 16K tokens) ([Wikipedia][9]).
  * **WizardCoder**: Un LLM de código mejorado aún más con fine-tuning por instrucciones, logrando resultados de primer nivel—incluso superando a algunos modelos propietarios—en tareas como HumanEval ([arXiv][10]).
...
2. **Alineación con Instrucciones**
   ...
   * Si un modelo coder no está ajustado por instrucciones, puede sobresalir en completar código pero podría tener dificultades para seguir comandos detallados como "refactoriza esta función".

3. **Mejores Casos de Uso**

   * Los *modelos Instruct* son excelentes cuando necesitas una capacidad amplia para tareas (por ejemplo, "Explica este concepto", "Escribe un resumen" o "Genera pseudocódigo").
...
> En una discusión en Reddit sobre los modelos Qwen, alguien señaló que el modelo base ya maneja bien la codificación—pero la versión Coder probablemente supera en tareas de código, especialmente en tamaños de modelo equivalentes ([Get Business Grants][12], [arXiv][8], [Reddit][13]).
...
* **Usa un modelo Instruct** cuando tus tareas abarcan un amplio rango (texto, preguntas y respuestas, código).
* **Usa un modelo Coder** cuando tu enfoque esté en escribir, completar o editar código de forma precisa y consciente del contexto.
