...
lang: es
layout: post
title: Uso de Moment.js en JavaScript
translated: true
type: note
---

Te mostraré cómo usar las librerías `moment` y `moment-timezone` en JavaScript. Estas son librerías populares para manejar fechas y horas.
...
// Fecha y hora actual
const now = moment();
console.log(now.format('YYYY-MM-DD HH:mm:ss')); // ej., "2025-03-03 14:30:45"

// Analiza una fecha específica
const date = moment('2025-12-25');
console.log(date.format('MMMM Do YYYY')); // "December 25th 2025"

// Manipula fechas
const tomorrow = moment().add(1, 'days');
console.log(tomorrow.format('ddd, MMM D')); // ej., "Tue, Mar 4"
...
- `mm`: minutos (30)
- `ss`: segundos (45)
- `z`: abreviatura de zona horaria

Métodos útiles:
```javascript
// Manipulación de fechas
moment().add(7, 'days');     // Agrega tiempo
moment().subtract(2, 'hours'); // Resta tiempo
moment().startOf('month');   // Inicio del período
...
momentTimezone.tz.guess(); // Adivina la zona horaria del usuario
```

Notas:
1. Estas versiones (^2.14.1 y ^0.5.7) son bastante antiguas (de alrededor de 2016). Considera usar las últimas versiones a menos que tengas requisitos específicos de compatibilidad.
2. Moment.js ahora se considera un proyecto legacy en modo mantenimiento. Para nuevos proyectos, podrías considerar alternativas como `date-fns` o `luxon`.