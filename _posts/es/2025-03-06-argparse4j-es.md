...
Para usar `net.sourceforge.argparse4j`, una biblioteca de Java para analizar argumentos de línea de comandos inspirada en `argparse` de Python, sigue estos pasos:
...
Para otras herramientas de construcción como Gradle, consulta el [Maven Central Repository](https://central.sonatype.com/artifact/net.sourceforge.argparse4j/argparse4j) para la configuración equivalente.
...
- **Argumentos opcionales** (por ejemplo, `-t`, `--type`) con banderas, opciones, valores predeterminados y texto de ayuda.
- **Argumentos posicionales** (por ejemplo, `file`) con soporte opcional de longitud variable usando `.nargs("*")`.

**Ejemplo:**
```java
parser.addArgument("-t", "--type")
    .choices("SHA-256", "SHA-512", "SHA1")  // Restringir a estas opciones
    .setDefault("SHA-256")                  // Valor predeterminado si no se proporciona
...
### 4. **Analizar Argumentos de Línea de Comandos**
Analiza los argumentos de línea de comandos (normalmente pasados como `String[] args` desde tu método `main`) usando `parser.parseArgs()`. Envuelve esto en un bloque try-catch para manejar errores de análisis con gracia.
...
**Ejemplo:**
```java
String hashType = ns.getString("type");  // por ejemplo, "SHA-256"
List<String> files = ns.getList("file"); // Lista de nombres de archivos
...
- **Ayuda Automática**: Ejecuta tu programa con `-h` o `--help` para ver un mensaje de ayuda generado.
- **Subcomandos**: Usa `parser.addSubparsers()` para comandos complejos (por ejemplo, `git commit`, `git push`).
- **Anotaciones**: Vincula argumentos a campos de clase usando anotaciones (consulta el manual del usuario para detalles).

Para un uso más avanzado, consulta el [Manual del Usuario de Argparse4j](https://argparse4j.github.io/userguide.html).
